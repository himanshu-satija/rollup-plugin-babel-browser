...
test("does not babelify excluded code", async (t) => {
  const code = await generate("fixtures/exclusions/main.js", {
    exclude: "**/foo.js",
  });
...
test("allows transform-runtime to be used instead of bundled helpers, but throws when CommonJS is used", async (t) => {
  await t.throwsAsync(
    () =>
      generate("fixtures/runtime-helpers-commonjs/main.js", {
        babelHelpers: "runtime",
      }),
    {
...
test("correctly renames helpers (rollup/rollup-plugin-babel#22)", async (t) => {
  ...
  t.false(
    code.includes("babelHelpers_get get"),
    "helper was incorrectly renamed"
  );
});
            ...
            visitor: {
              FunctionDeclaration(path) {
                ...
                path
                  ...
                  .replaceWith(
                    types.identifier(
                      String.fromCharCode(BASE_CHAR_CODE + counter)
                    )
                  );
...
test("supports overriding the plugin options in custom loader", async (t) => {
  ...
  t.false(
    code.includes("console.foobaz"),
    "does not run the plugin on ignored file"
  );
});
